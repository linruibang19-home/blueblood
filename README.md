...
> 📄 **完整项目文档**(业务架构 / 技术架构 / 数据库 / 部署 / 完成度 / **简历素材** / **面试问答**)见 [`docs/`](docs/README.md)
...
### 核心业务流（任务交付闭环）
```
企业/个人发布任务(配多里程碑,各带金额) → 用户接单(生成订单 + 复制里程碑实例)
   → 接单者提交里程碑(成果+附件) → 雇主(发布者)审核(APPROVED/REJECTED)
   → APPROVED 即按里程碑金额入账接单者钱包(分阶段结算,幂等)
   → 全部里程碑通过 → 订单 settled
   → 用户提现申请 → 管理员审核 → 打款 / 驳回退款
```

### 角色
- **个人用户（USER / personal）**：浏览社区、学习课程、接单交付、发布任务、收益提现。
- **企业用户（USER / enterprise，认证审核后）**：个人全部能力 + 发布岗位/黑客松/任务 + 雇主自助验收里程碑。
- **管理员（ADMIN）**：后台运营——用户/认证审核、内容管理、任务/接单/里程碑审核、收益结算、系统配置。
...
- `sql/init.sql`：建库 + 44+ 表 + 种子数据（含完整的 接单→里程碑→提交→审核→结算 演示链）。
- `sql/v2_001~v5_001`：增量迁移 —— v2 RBAC/认证、v3 企业角色、v4 任务发布+里程碑分阶段结算、v5 课程种子。
- 导入：`mysql -u root -p < sql/init.sql`（Docker 方式自动导入 init.sql；v2~v5 为已运行库的增量迁移）。
